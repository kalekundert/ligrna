...
plate_1 = Plate([
    PlateInfo('TEP_conc', 0.0, ['A1', 'D3', 'D4']),
    PlateInfo('TEP_conc', 0.25e-3, ['A2']), # 0.25 mM
    PlateInfo('TEP_conc', 0.5e-3, ['A3']), # 0.5 mM
    PlateInfo('TEP_conc', 0.75e-3, ['A4']), # 0.75 mM
    PlateInfo('TEP_conc', 1.0e-3, ['A5', 'D1', 'D2']), # 1 mM

    PlateInfo('IPTG_conc', 1.0e-3, ['A1-A5', 'D1-D4']), # 1 mM

    PlateInfo('ATC_conc', 1.0e-6, ['A1-A5', 'D1-D4']), # 1uM

    PlateInfo('Control-Pos', None, ['D1', 'D3']),
    PlateInfo('Control-Neg', None, ['D2', 'D4']),
    PlateInfo('NX-0', None, ['A1-A5']),
], sample_dir = '/home/kyleb/data/cas9/151106/kyleb/151106-4pm_002_sgrna/96 Well - Flat bottom',
    name = 'replicate_1',
)

plate_2 = Plate([
    PlateInfo('TEP_conc', 0.0, ['B1', 'E3', 'E4']),
    PlateInfo('TEP_conc', 0.25e-3, ['B2']), # 0.25 mM
    PlateInfo('TEP_conc', 0.5e-3, ['B3']), # 0.5 mM
    PlateInfo('TEP_conc', 0.75e-3, ['B4']), # 0.75 mM
    PlateInfo('TEP_conc', 1.0e-3, ['B5', 'E1', 'E2']), # 1 mM

    PlateInfo('IPTG_conc', 1.0e-3, ['B1-B5', 'E1-E4']), # 1 mM

    PlateInfo('ATC_conc', 1.0e-6, ['B1-B5', 'E1-E4']), # 1uM

    PlateInfo('Control-Pos', None, ['E1', 'E3']),
    PlateInfo('Control-Neg', None, ['E2', 'E4']),
    PlateInfo('NX-0', None, ['B1-B5']),
], sample_dir = '/home/kyleb/data/cas9/151106/kyleb/151106-4pm_002_sgrna/96 Well - Flat bottom',
    name = 'replicate_2',
)

plate_3 = Plate([
    PlateInfo('TEP_conc', 0.0, ['C1', 'F3', 'F4']),
    PlateInfo('TEP_conc', 0.25e-3, ['C2']), # 0.25 mM
    PlateInfo('TEP_conc', 0.5e-3, ['C3']), # 0.5 mM
    PlateInfo('TEP_conc', 0.75e-3, ['C4']), # 0.75 mM
    PlateInfo('TEP_conc', 1.0e-3, ['C5', 'F1', 'F2']), # 1 mM

    PlateInfo('IPTG_conc', 1.0e-3, ['C1-C5', 'F1-F4']), # 1 mM

    PlateInfo('ATC_conc', 1.0e-6, ['C1-C5', 'F1-F4']), # 1uM

    PlateInfo('Control-Pos', None, ['F1', 'F3']),
    PlateInfo('Control-Neg', None, ['F2', 'F4']),
    PlateInfo('NX-0', None, ['C1-C5']),
], sample_dir = '/home/kyleb/data/cas9/151106/kyleb/151106-4pm_002_sgrna/96 Well - Flat bottom',
    name = 'replicate_3',
)
...
def plot_gate_value(gate_name=None, gate_val=None, outer_fig_dir=None):
    ...
    diffs_fig.suptitle('2015/11/06 - Mean biological replicate fold signal over TEP 0.0 conc. (IPTG 1mm) (mean of replicate means of gated distributions)', y=1.04)
