...
plate_1 = Plate([
    PlateInfo('TEP_conc', 0.0, ['A1-C1',]),
    PlateInfo('TEP_conc', 0.25e-3, ['A2-C2']), # 0.25 mM
    PlateInfo('TEP_conc', 0.5e-3, ['A3-C3']), # 0.5 mM
    PlateInfo('TEP_conc', 0.75e-3, ['A4-C4']), # 0.75 mM
    PlateInfo('TEP_conc', 1.0e-3, ['A5-C5']), # 1 mM

    PlateInfo('IPTG_conc', 1.0e-3, ['A1-C5']), # 1 mM

    PlateInfo('ATC_conc', 1.0e-6, ['A1-C5']), # 1uM

    PlateInfo('Control-Pos', None, ['A1-A5']),
    PlateInfo('Control-Neg', None, ['B1-B5']),
    PlateInfo('NX-0', None, ['C1-C5']),
], sample_dir = '/kortemmelab/data/kyleb/cas9/151208/151208-61halfhour/96 Well - Flat bottom',
    name = 'replicate_1',
)

plate_2 = Plate([
    PlateInfo('TEP_conc', 0.0, ['D1',]),
    PlateInfo('TEP_conc', 0.25e-3, ['D2']), # 0.25 mM
    PlateInfo('TEP_conc', 0.5e-3, ['D3']), # 0.5 mM
    PlateInfo('TEP_conc', 0.75e-3, ['D4']), # 0.75 mM
    PlateInfo('TEP_conc', 1.0e-3, ['D5']), # 1 mM

    PlateInfo('IPTG_conc', 1.0e-3, ['D1-D5']), # 1 mM

    PlateInfo('ATC_conc', 1.0e-6, ['D1-D5']), # 1uM

    PlateInfo('NX-0', None, ['D1-D5']),
], sample_dir = '/kortemmelab/data/kyleb/cas9/151208/151208-61halfhour/96 Well - Flat bottom',
    name = 'replicate_2',
)

plate_3 = Plate([
    PlateInfo('TEP_conc', 0.0, ['E1',]),
    PlateInfo('TEP_conc', 0.25e-3, ['E2']), # 0.25 mM
    PlateInfo('TEP_conc', 0.5e-3, ['E3']), # 0.5 mM
    PlateInfo('TEP_conc', 0.75e-3, ['E4']), # 0.75 mM
    PlateInfo('TEP_conc', 1.0e-3, ['E5']), # 1 mM

    PlateInfo('IPTG_conc', 1.0e-3, ['E1-E5']), # 1 mM

    PlateInfo('ATC_conc', 1.0e-6, ['E1-E5']), # 1uM

    PlateInfo('NX-0', None, ['E1-E5']),
], sample_dir = '/kortemmelab/data/kyleb/cas9/151208/151208-61halfhour/96 Well - Flat bottom',
    name = 'replicate_3',
)
...
def main():
    ...
    if True: # fast_run
        plot_gate_value('poly_0.8', 0.8, outer_fig_dir)
    else:
        func_args = []
