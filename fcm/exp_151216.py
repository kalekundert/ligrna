...
plate_1 = Plate([
    PlateInfo('TEP_conc', 0.0, ['A3-C3',]),
    PlateInfo('TEP_conc', 1.0e-3, ['A2-C2']), # 1 mM
    PlateInfo('TEP_conc', 4.59e-3, ['A1-C1']), # 4.59 mM

    PlateInfo('IPTG_conc', 1.0e-3, ['A1-C3']), # 1 mM

    PlateInfo('ATC_conc', 1.0e-6, ['A1-C3']), # 1uM

    PlateInfo('Control-positive', None, ['B1-B3']),
    PlateInfo('Control-sgRNA-null', None, ['C1-C3']),
    PlateInfo('sgRNA-cb', None, ['A1-A3']),
], sample_dir = '/kortemmelab/data/kyleb/cas9/151216-tep/96 Well - Flat bottom',
    name = 'replicate_1',
)

plate_2 = Plate([
    PlateInfo('TEP_conc', 0.0, ['A6-C6',]),
    PlateInfo('TEP_conc', 1.0e-3, ['A5-C5']), # 1 mM
    PlateInfo('TEP_conc', 4.59e-3, ['A4-C4']), # 4.59 mM

    PlateInfo('IPTG_conc', 1.0e-3, ['A4-C6']), # 1 mM

    PlateInfo('ATC_conc', 1.0e-6, ['A4-C6']), # 1uM

    PlateInfo('Control-positive', None, ['B4-B6']),
    PlateInfo('Control-sgRNA-null', None, ['C4-C6']),
    PlateInfo('sgRNA-cb', None, ['A4-A6']),
], sample_dir = '/kortemmelab/data/kyleb/cas9/151216-tep/96 Well - Flat bottom',
    name = 'replicate_2',
)

plate_3 = Plate([
    PlateInfo('TEP_conc', 0.0, ['A9-C9',]),
    PlateInfo('TEP_conc', 1.0e-3, ['A8-C8']), # 1 mM
    PlateInfo('TEP_conc', 4.59e-3, ['A7-C7']), # 4.59 mM

    PlateInfo('IPTG_conc', 1.0e-3, ['A7-C9']), # 1 mM

    PlateInfo('ATC_conc', 1.0e-6, ['A7-C9']), # 1uM

    PlateInfo('Control-positive', None, ['B7-B9']),
    PlateInfo('Control-sgRNA-null', None, ['C7-C9']),
    PlateInfo('sgRNA-cb', None, ['A7-A9']),
], sample_dir = '/kortemmelab/data/kyleb/cas9/151216-tep/96 Well - Flat bottom',
    name = 'replicate_3',
)
