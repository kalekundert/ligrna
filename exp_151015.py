...
plate_1 = Plate([
    PlateInfo('TEP_conc', 0.0, ['A1-D1', 'A4-D4', 'A7-D7', 'A10-D10']),
    PlateInfo('TEP_conc', 0.5e-3, ['A2-D2', 'A5-D5', 'A8-D8', 'A11-D11']), # 0.5 mM
    PlateInfo('TEP_conc', 1.0e-3, ['A3-D3', 'A6-D6', 'A9-D9', 'A12-D12']), # 1 mM

    PlateInfo('IPTG_conc', 0.0, ['A1-D3', 'A7-D9']),
    PlateInfo('IPTG_conc', 1.0e-3, ['A4-D6', 'A10-D12']), # 1 mM

    PlateInfo('ATC_conc', 0.0, ['A1-D6']),
    PlateInfo('ATC_conc', 1.0e-6, ['A7-D12']), # 1uM

    PlateInfo('Control-Pos', None, 'A1-A12'),
    PlateInfo('Control-Neg', None, 'B1-B12'),
    PlateInfo('NX-0', None, 'C1-C12'),
    PlateInfo('US-0,0', None, 'D1-D12'),
], sample_dir = '/kortemmelab/data/kyleb/cas9/151015/kyleb/151015-rfp-cas9-4,3a,50pm/Replicates 1 and 2',
    name = 'replicate_1',
)

plate_2 = Plate([
    PlateInfo('TEP_conc', 0.0, ['E1-H1', 'E4-H4', 'E7-H7', 'E10-H10']),
    PlateInfo('TEP_conc', 0.5e-3, ['E2-H2', 'E5-H5', 'E8-H8', 'E11-H11']), # 0.5 mM
    PlateInfo('TEP_conc', 1.0e-3, ['E3-H3', 'E6-H6', 'E9-H9', 'E12-H12']), # 1 mM

    PlateInfo('IPTG_conc', 0.0, ['E1-H3', 'E7-H9']),
    PlateInfo('IPTG_conc', 1.0e-3, ['E4-H6', 'E10-H12']), # 1 mM

    PlateInfo('ATC_conc', 0.0, ['E1-H6']),
    PlateInfo('ATC_conc', 1.0e-6, ['E7-H12']), # 1uM

    PlateInfo('Control-Pos', None, 'E1-E12'),
    PlateInfo('Control-Neg', None, 'F1-F12'),
    PlateInfo('NX-0', None, 'G1-G12'),
    PlateInfo('US-0,0', None, 'H1-H12'),
], sample_dir = '/kortemmelab/data/kyleb/cas9/151015/kyleb/151015-rfp-cas9-4,3a,50pm/Replicates 1 and 2',
    name = 'replicate_2',
)
...
plate_3 = Plate([
    ...
    PlateInfo('NX-0', None, 'C1-C12'),
    PlateInfo('US-0,0', None, 'D1-D12'),
    PlateInfo('Control-Blank', None, 'E1'),
], sample_dir = '/kortemmelab/data/kyleb/cas9/151015/kyleb/151015-rfp-cas9-4,3a,50pm/Replicate 3 and blank',
    name = 'replicate_3',
)
...
def main():
    outer_fig_dir = os.path.join('script_output', 'exp_151015')
...
def plot_gate_value(gate_name=None, gate_val=None, outer_fig_dir=None):
    ...
    for plate_num, exp in enumerate(all_plates):
        ...
        pos_wells = list(exp.well_set('Control-Pos').intersection(atc_wells).intersection(iptg_wells).intersection(tep_wells))
        assert( len(pos_wells) == 1 )
        neg_wells = list(exp.well_set('Control-Neg').intersection(atc_wells).intersection(iptg_wells).intersection(tep_wells))
        assert( len(neg_wells) == 1 )
        for all_channel_name in exp.samples[pos_wells[0]].data:
            print plate_num, all_channel_name, '%.1f' % (exp.samples[neg_wells[0]].data[all_channel_name].mean() / exp.samples[pos_wells[0]].data[all_channel_name].mean()), '%.1f' % exp.samples[pos_wells[0]].data[all_channel_name].mean(), '%.1f' % exp.samples[neg_wells[0]].data[all_channel_name].mean()
        print
    #     continue
    # sys.exit(0)
